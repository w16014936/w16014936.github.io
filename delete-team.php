...
if(isset($_REQUEST['team_id'])){
    ...
    if(is_numeric($_REQUEST['team_id'])){
        if(!empty(getTeam($dbConn, $_REQUEST['team_id']))){
            ...
            $errorText = "You have not chosen a valid team to delete.  Please select a team <a href='manage-team.php'>here</a> to remove it from the system";
        
        }

    } else {
        $errorText = "You have not chosen a valid team to delete.  Please select an team <a href='manage-team.php'>here</a> to remove it from the system";
...
<div class="container">
  <div class="row justify-content-center align-items-center">
    ...
    if (isset($errorText)){
      ...
      // Validate
      list($input, $errors) = validateDeleteTeamForm($dbConn);
    
      if (empty($errors)){
        // Update
        if (deleteTeam($dbConn, $input)){
          $deleteSuccess = "<h3>You have successfully deleted the team. To manage another please click <a href='manage-team.php'>here</a>.</h3>";
        }
    ...
    if (isset($input, $errors) && !isset($deleteSuccess)){
      ...
      <div class="col-sm-6">
        <h3 class="text-center">You are about to delete <?= $team_name; ?></h3>
        <form action="delete-team.php" name="deleteForm" method="POST">
