...
if(isset($_REQUEST['account_id'])){
    ...
    if(is_numeric($_REQUEST['account_id'])){
        if(!empty(getAccount($dbConn, $_REQUEST['account_id']))){
            ...
            $errorText = "You have not chosen a valid account to restore.  Please select an account <a href='manage-account.php'>here</a> to remove it from the system";
...
<div class="container">
  <div class="row justify-content-center align-items-center">
    ...
    if (isset($errorText)){
      ...
      // Validate
      list($input, $errors) = validateRestoreAccountForm($dbConn);
    
      if (empty($errors)){
        // Update
        if (restoreAccount($dbConn, $input)){
          $restoreSuccess = "<h3>You have successfully restored the account. To manage another please click <a href='manage-account.php'>here</a>.</h3>";
        }
    ...
    if (isset($input, $errors) && !isset($restoreSuccess)){
      ?>
      <div class="col-sm-3"></div>
      <div class="col-sm-6">
        <h3 class="text-center">You are about to restore <?= $username; ?></h3>
        <form action="restore-account.php" name="restoreForm" method="POST">
          ...
          <button type="submit" id="restore-button" class="btn btn-primary">RESTORE</button>
        </form>
      </div>
      <div class="col-sm-3"></div>
